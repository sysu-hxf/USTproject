...
def check_winner(board):
    ...
    # 检查情况3：对角线方向
    for x1 in range(16):
        for y in range(1,10 - 4):  # 限制y范围，确保y+4不越界
            player = board[x1, y]
            if player != 0:
                count = 0
                for k in range(5):
...
def computer_move_4(board, color):
    ...
    class OpeningBook:
        ...
        def get_opening_move(self, board, color, style='balanced'):
            ...
            # 如果是第一步，随机选择一个开局策略
            if move_count == 0:
                if random.random() < 0.8:  # 30%概率使用中心点开局
                    return (8, 0)
                else:
        ...
        def evaluate_pattern(self, line, color):
            ...
            for i in range(length - 5):
                ...
                # 特殊活三 (01010)
                        ...
                        line[i + 3] == color and
                        line[i + 4] == 0):
                    score += SCORE_THREE * 0.8  # 略低于标准活三的分数
    ...
    def simulate_game(board, color, strategy, max_depth=10):
        """模拟游戏"""
        ...
        # 评估最终局面
        antimove = get_simulation_move(current_board, -color, strategy)
        final_score = strategy.evaluate_position(current_board, antimove, color)
    ...
    def select_bettermove(board, color, strategy,moves):
        bettermoves = []
        for coord in moves:
                i,j = coord
                if board[i, j] == 0:
                    score = strategy.evaluate_position(board, (i, j), color)
                    bettermoves.append(((i, j), score))
        bettermoves.sort(key=lambda x: x[1], reverse=True)
        return bettermoves[0][0]
    ...
    def check_33_position(board, color):
        BOARD_SIZE_ANGULAR = 16
        BOARD_SIZE_RADIAL = 10

        def is_valid_position(x, y):
            return 0 <= x < BOARD_SIZE_ANGULAR and 0 <= y < BOARD_SIZE_RADIAL

        def get_circular_index(x):
            if x>=0:
                tmp = x % BOARD_SIZE_ANGULAR
                return int(tmp)
            if x<0:
                tmp = x+BOARD_SIZE_ANGULAR
                return int(tmp)

        def check_three_in_line(x, y, dx, dy):
            count = 0
            # 检查前两个位置
            for i in range(1, 3):
                new_x = get_circular_index(x + dx * i)
                new_y = y + dy * i
            ...
            for i in range(1, 3):
                ...
                if board[new_x, new_y] == color:
                    count += 1
            return count >= 2  # 如果有两个或更多相连的棋子，就可能形成三连
        ...
        for i in range(BOARD_SIZE_ANGULAR):
            for j in range(BOARD_SIZE_RADIAL):
                ...
                # 检查每个方向
                for dx, dy in directions:
                    if abs(dy) == 1 and dx == 1: continue
            
                    if check_three_in_line(i, j, dx, dy):
                        three_count += 1
    ...
    def check_3_position(board, color,alpha=2):
        BOARD_SIZE_ANGULAR = 16
        BOARD_SIZE_RADIAL = 10
        ...
        def check_three_in_line(x, y, dx, dy):
            ...
            return count >= alpha  # 默认alpha=2时如果有两个或更多相连的棋子，就可能形成三连

        def check_three_in_line_dia(x, y, dx, dy):
            count = 0
            # 检查前两个位置
            for i in range(1, 3+alpha-2):
                new_x = get_circular_index(x + dx * i)
                new_y = y + dy * i
                if not is_valid_position_dia(new_x, new_y):
                    continue
                if board[new_x, new_y] == color and new_y != 0:
                    count += 1
                else:
                    break
            # 检查后两个位置
        ...
        for i in range(BOARD_SIZE_ANGULAR):
            for j in range(BOARD_SIZE_RADIAL):
                ...
                for dx, dy in dia_directions:      
                    if check_three_in_line_dia(i, j, dx, dy):
                        results.append((i, j))
                    board[i, j] = 0
        return results

    def check_4_position(board, color):
        BOARD_SIZE_ANGULAR = 16
        BOARD_SIZE_RADIAL = 10
        ...
        def check_four_in_line(x, y, dx, dy):
            count = 0
            # 检查前两个位置
            for i in range(1, 4):
                new_x = get_circular_index(x + dx * i)
            ...
            for i in range(1, 4):
                ...
                if board[new_x, new_y] == color:
                    ...
                    break
            return count >= 3  # 如果有两个或更多相连的棋子，就可能形成三连

        def check_four_in_line_dia(x, y, dx, dy):
            count = 0
            # 检查前两个位置
            for i in range(1, 4):
                new_x = get_circular_index(x + dx * i)
        ...
        for i in range(BOARD_SIZE_ANGULAR):
            for j in range(BOARD_SIZE_RADIAL):
                ...
                # 检查每个方向
                for dx, dy in ud_directions:
                    if check_four_in_line(i, j, dx, dy):
                        results.append((i, j))
                    # 移除临时放置的棋子
                    board[i, j] = 0

                for dx, dy in dia_directions:    
                    if check_four_in_line_dia(i, j, dx, dy):
                        results.append((i, j))
                    board[i, j] = 0
...
#    try:
    ...
    if blocking_move:
        ...
        return blocking_move
    # 3. 检查必须防守的潜在四连威胁
    four_threat = check_4_position(board, -color)
    if four_threat:
        print("有潜在的四连:")
        return four_threat[0]
    #4. 检查对手已有三连
    three_threat = check_3_position(board, -color, alpha = 3)
    if three_threat:
        print("对手已有无阻挡三连，我必须下： ", three_threat)
        return three_threat[0]
    #5. atk. 检查己方四子机会
    winchance_move = check_4_position(board, color)
    if winchance_move:
        print("better win chance:")
        return select_bettermove(board, color, strategy, winchance_move)
    #6. 检查双三
    three_threat = check_33_position(board, -color)
    if three_threat:
        print("有双三连，我决定下： ", three_threat)
        return three_threat[0]
    #7. 检查对手潜在三连机会
    three_threat = check_3_position(board, -color)
    if three_threat:
        print("有三连，我决定下： ", three_threat)
        return select_bettermove(board, -color, strategy, three_threat)
...
if __name__ == '__main__':
    #battle(computer_move_4,computer_move_1)
    main()
