    ...
    class PositionEvaluator:
        ...
        def evaluate_pattern(self, line, color):
            ...
            for i in range(length - 3):
                ...
                # 防止对手33策略
            for i in range(length - 4):
                if (line[i] == 0 and line[i + 1] == 0 and
                        line[i + 2] == color and line[i + 3] == color and
                        line[i + 4] == color):
                    score -= SCORE_BLOCKED_THREE * 2  # 对手可能的33组合

            # 防止对手34策略
            for i in range(length - 4):
                if (line[i] == 0 and line[i + 1] == color and
                        line[i + 2] == 0 and line[i + 3] == color and
                        line[i + 4] == color):
                    score -= SCORE_BLOCKED_THREE * 2  # 对手可能的34组合
...
if __name__ == '__main__':
    #battle(computer_move_1,computer_move_4)
    main()
